...
class TextRank(LoadFile):
    ...
    def build_word_graph(self, window=2, pos=None):
        ...
        TF_dict={}
        for (word1,word2) in text2:
            if (word1,word2) in TF_dict.keys():
                TF_dict[(word1,word2)]=(TF_dict[(word1,word2)]+1)
            else:
                TF_dict[(word1,word2)]=1
        n=len(text2)
        ...
        for (word1,word2) in TF_dict.keys():
            count=0.0
            for each in word3:
              for i in range(0,len(each)-1):
                if word1==each[i] and word2==each[i+1]:
                  count=count+1.0
                  break

            idf_dict[(word1,word2)]=(math.log((N)/(count)))


        for (word1,word2) in TF_dict1.keys():
            TFIDF_dict[(word1,word2)]=(float)(TF_dict1[(word1,word2)])*(float)(idf_dict[(word1,word2)])
        ...
        for i in range(0,len(text2)-1):
            ...
            for j in range(0,len(text2)-1):
                if(text2[i][0]==text2[j][0] and text2[i][1]==text2[j][1] and text2[i+1][0]==text2[j+1][0] and text2[i+1][1]==text2[+1][1] ):
                    weight1[text2[i],text2[i+1]]+=1
        ...
        for i, (node1, node2) in enumerate(text2):
            ...
            for j in range(i + 1, min(i + window, len(text2))):
                (node3, node4) = text2[j]
                if node2 == node3 and not self.graph.has_edge((node1, node2),(node3, node4)):
                    self.graph.add_edge((node1, node2),(node3, node4),weight=weight1[(node1, node2),(node3, node4)])
                    self.graph[(node1, node2)][(node3, node4)]['weight'] += 0.0
    ...
    def pagerank(self,alpha=0.85,personalization=None,max_iter=100,tol=1.0e-6,nstart=None,weight='weight',dangling=None):
      ...
      #print('W_nodes')
      #angling_nodes1 = [n for n in W]
      #print(dangling_nodes1)
...
keyphrasesdata.to_csv('Data/'+ fileinput +'/keyphrases_original_biword.csv',index=False)
