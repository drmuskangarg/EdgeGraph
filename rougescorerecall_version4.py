...
def rouge_1(extracted,actual):
    ...
    if len(refdata)>0:
        result=(float)((float)(getval)/len(refdata))
    else:
        result=0
    #print (result)
    return result
...
def dataframe(filename):
    ...
    for eachfilename1 in listoffile1:
        ...
        i+=1
    return df6
def rouge_1_Recall(no_keyphrases,file):
...
dict5={}
for k in range(1,21):
    dict4=rouge_1_Recall(k,filename_1)
    dict5[k]=dict4
...
plt.ylabel("Recall")
plt.suptitle("Recall for 100thesis Data Set")
...
plt.savefig('Data/Results/Rouge_1_Recall_Score for thesis100.png', dpi=300,bbox_inches='tight')
plt.show()
df4.to_csv('Data/Results/results_recall_thesis100.csv')
